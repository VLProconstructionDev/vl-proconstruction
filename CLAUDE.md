...
src/
  ...
  lib/blog.ts              — post sorting/draft filter, read-time + "New" badge helpers
  ...
  pages/
    ...
    blog/
      index.astro                        — all posts (hand-designed, .bl-* cards)
      [slug].astro                       — thin route → BlogPostLayout
...
**`blog`** — slug = filename → URL. Fields: `title`, `seoTitle?` (shorter `<title>` when the h1 runs long — the h1 always uses `title`), `description`, `publishedAt`, `updatedAt?`, `author`, `heroImage?`, `authorImage?`, `tags[]`, `relatedService?`, `relatedLocation?`, `draft` (draft posts visible in dev, hidden in prod). One live post: `tub-to-shower-conversion-bradenton` (~1,900 words, written from a keyword-gap brief — Angi cost data, the curbed/curbless comparison table, an ADA §608 checklist, Manatee/Sarasota permit rules, 6 FAQs; hero is a real job photo from `gallery-showers/`). Frontmatter strings containing `: ` must be quoted or the YAML parse fails. There is no `readTime` field: `src/lib/blog.ts` computes it from the body at ~200 wpm, and the "New" ribbon is derived from `publishedAt` (60 days) — `PostCard`'s `showNew={false}` turns it off on `/blog`, where every recent post would wear one.
...
- `/blog` — all posts; `/blog/[slug]` — post pages (1 live)
- `/404` — branded 404
...
### Blog (`/blog`, `/blog/[slug]`)
`src/pages/blog/index.astro` (hand-designed list) + `src/pages/blog/[slug].astro` (thin route) + `src/layouts/BlogPostLayout.astro` (the whole post page) + the `blog` collection + `src/lib/blog.ts` (`sortPosts`, the `visible` draft filter, `readTime`, `isNew`, `monthYear`/`fullDate` formatters — imported by all three lists so the home teaser, `/blog`, and "Keep reading" agree).

Post cards are one component — `PostCard.astro` (styles: the shared `.bl-*` rules in `global.css`) — rendered by the home-page `#advice` teaser, `/blog`, and "Keep reading", so the three lists cannot drift. `PostMeta.astro` is the meta row (also used by the post header with `dateFormat="full"`): date · author avatar + name · clock + read time, plus a `Draft` chip in dev. The "New" flag is a `.bl-ribbon` on the photo's top-right corner, driven by `isNew(publishedAt)`. An author with no `authorImage` gets the brand mark on a dark disc (`favicon.png` is white line art, hence the disc); setting `authorImage` swaps in a real headshot that fills the circle. Post pages break the site's full-bleed grid on purpose: a centred `.post-wrap` (1180px) on a `#f6f5f3` ground, laid out like a magazine page — a title band (category chip from the post's first tag · `h1` · `PostMeta variant="post"` · standfirst) closed by a hairline, then a two-column grid on `lg`: the article column (hero image inside it, then `.post-body` prose) beside a **sticky right rail** (`top-112px`, desktop-only) of three cards — a white `.rail-cta` estimate card (the brand button carries it), an "In this article" TOC card built from the post's `h2` headings (`render()`'s `headings`; Astro slugs the ids), and a "Questions?" card with the phone number. A small inline `IntersectionObserver` paints the current TOC link (`.toc-link.is-active`). The rail is hidden on phones, where it would only push the article down and the sticky mobile CTA already covers the action. Below that, on white: "Keep reading" (the three newest other posts), then `<EstimateInline />` — both blog pages end with the real form rather than a CTA band. `relatedService` in a post's frontmatter renders a link back to that service page.
...
The `services`/`locations` collections and `ServiceLayout.astro` remain in place (typed, seeded with Suncoast cities) so releasing is a file move, not a rebuild.
...
`hero` (dark), `services` (light), `consultation` (light), `benefits` (light), `about` (light), `process` (light), `gallery` (dark), `before-after` (light), `reviews` (light), `service-area` (dark), `estimate` (dark — `<EstimateInline />`), `faqs` (light), `advice` (light — the three newest blog posts + a "See more blogs" pill to `/blog`), plus footer (dark).
...
- **New blog post**: create `src/content/blog/foo.md` with `draft: false` → `/blog/foo`, and it appears on `/blog` + the home-page `#advice` teaser (newest three) automatically. Drafts render in dev only, and post pages carry `noindex` while `draft: true`.
